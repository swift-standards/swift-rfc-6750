...
extension RFC_6750.Bearer {
    ...
    public static func parse(fromFormParameters parameters: [String: String]) throws -> RFC_6750.Bearer {
    ...
    public static func parse(fromQueryItems queryItems: [URLQueryItem]) throws -> RFC_6750.Bearer {
        guard let tokenItem = queryItems.first(where: { $0.name == "access_token" }),
              let tokenString = tokenItem.value else {
...
extension RFC_6750.Bearer {
    ...
    public struct Challenge: Codable, Hashable, Sendable {
        ...
        public init(realm: String? = nil, scope: String? = nil, error: ErrorCode? = nil, errorDescription: String? = nil) {
            self.realm = realm
            self.scope = scope
            self.error = error
        ...
        public static func parse(from headerValue: String) throws -> RFC_6750.Bearer.Challenge {
            ...
            let parameters = String(trimmed.dropFirst(6)).trimmingCharacters(in: .whitespacesAndNewlines)
            var realm: String?
            var scope: String?
            var error: ErrorCode?
            ...
            if !parameters.isEmpty {
                ...
                for component in components {
                    ...
                    if trimmedComponent.lowercased().hasPrefix("realm=") {
                        ...
                        if let errorValue = extractQuotedValue(from: trimmedComponent, parameter: "error") {
                            error = ErrorCode(rawValue: errorValue)
                        }
                    } else if trimmedComponent.lowercased().hasPrefix("error_description=") {
                        errorDescription = extractQuotedValue(from: trimmedComponent, parameter: "error_description")
                    }
                }
            }
            
            return RFC_6750.Bearer.Challenge(realm: realm, scope: scope, error: error, errorDescription: errorDescription)
        }
        ...
        private static func extractQuotedValue(from component: String, parameter: String) -> String? {
            ...
            let value = String(component.dropFirst(prefix.count)).trimmingCharacters(in: .whitespacesAndNewlines)
...
extension RFC_6750.Bearer {
    ...
    public enum ErrorCode: String, Codable, Hashable, Sendable, CaseIterable {
        ...
        public var description: String {
            switch self {
            case .invalidRequest:
                return "The request is missing a required parameter, includes an unsupported parameter or parameter value, repeats the same parameter, uses more than one method for including an access token, or is otherwise malformed."
            case .invalidToken:
